  ...
  const fetchData = async () => {
    try {
      ...
      const groupedTransactions = resTrans.reduce((acc, trans) => {
        const date = DateUtil.ShortFormat(trans.transactionDate);
        if (!acc[date]) {
          acc[date] = {
            data: [],
            total: 0,
        ...
        acc[date].data.push(trans);
        acc[date].total += trans.totalPrice;
        return acc;
      }, {});
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <View className="h-screen mt-12">
        <View className="px-3">
          ...
          <Animated.View entering={FadeIn.delay(250)} className="mb-4">
            ...
            >
              <TouchableOpacity
                ...
                style={{
                  backgroundColor:
                    selectedCategory === null ? "#2f3060" : "#f0efef",
                }}
              >
                <Icon
                  name={"albums-outline"}
                ...
                />
                <Text
                  className={`font-semibold text-[16px] ${selectedCategory === null ? "text-white" : "text-neutral-700"}`}
                >
                  Semua Category
                </Text>
              </TouchableOpacity>
              {categories.map((cat, index) => {
                return (
                  <TouchableOpacity
                    ...
                    style={{
                      backgroundColor:
                        selectedCategory?.id === cat.id ? "#2f3060" : "#f0efef",
                    }}
                  >
                    <Icon
                      name={cat.icon}
                      color={
                        selectedCategory?.id === cat.id ? "white" : cat.color
                      }
                      size={16}
                      className="mr-1"
                    />
          ...
          <Animated.View
            ...
            className="flex flex-row justify-evenly items-center gap-2 mb-2"
          >
            <TouchableOpacity
              activeOpacity={AppStyle.TouchableOpacity.Active}
              onPress={() => navigate.navigate("Summary")}
              className="bg-orange-500 py-3 w-1/2 rounded-lg flex flex-row justify-center items-center gap-x-1"
            >
            ...
            <TouchableOpacity
              ...
              className="bg-green-500 py-3 w-1/2 rounded-lg flex flex-row justify-center items-center gap-x-1"
            >
          ...
          <View className=" h-[475px]">
            {Object.keys(transactions).length > 0 ? (
                ...
                {Object.keys(transactions).map((date, index) => (
                  <Animated.View entering={FadeIn.delay(200)} key={index}>
                    <View className="flex flex-wrap flex-row justify-between items-center">
                      <Text className="text-neutral-700 font-bold mb-4 text-[20px]">
                        {date}
                      </Text>
                      <Text className="text-neutral-700 font-bold mb-4 text-[18px]">
                        {FormatRupiahWithRp(transactions[date].total)}
                      </Text>
                    </View>
                    {transactions[date].data.map((trans, idx) => (
                      <TouchableOpacity
                        activeOpacity={AppStyle.TouchableOpacity.Active}
                        onPress={() => handleSheetUpdate(trans)}
                        key={idx}
                        className={`bg-blue-500 w-full my-4 flex flex-row justify-start items-center gap-y-2 px-2 pt-1 pb-3 rounded-lg`}
                      >
                        <View>
                          ...
                          <Text className="text-white font-semibold text-[17px]">
                            {FormatRupiahWithRp(trans.totalPrice)} - (
                            {trans.quantity})
                          </Text>
                        </View>
                      </TouchableOpacity>
              ...
              <Animated.View
                entering={FadeIn.delay(400)}
                className="pb-4 rounded-lg bg-white flex justify-center items-center"
              >
                <Image
      ...
      <BottomSheetTransaction
        ...
        fetch={fetchData}
        db={db}
        refRBSheet={refRBSheetAdd}
      />
      <BottomSheetTransactionUpdate
        transaction={transactionUpdate}
        categories={categories}
        fetch={fetchData}
        db={db}
        refRBSheet={refRBSheetUpdate}
      />
    </View>
  );
}
